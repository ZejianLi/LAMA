...
def create_inception_graph(pth):
    ...
    # with tf.gfile.GFile(pth, 'rb') as f: # TF 1.x
    with tf.io.gfile.GFile( pth, 'rb') as f: # TF 2.x
        # graph_def = tf.GraphDef() # TF 1.x
        graph_def = tf.compat.v1.GraphDef() # TF 2.x
...
def load_image_batch(files):
    ...
    """
    return np.stack([process(str(fn)).astype(np.float32) for fn in files], axis=0)
...
def get_activations_from_files(files, sess, batch_size=50, verbose=False):
    ...
    if batch_size > n_imgs:
        ...
        batch_size = n_imgs
    n_images = n_imgs
    n_batches = n_imgs//batch_size + int(n_images%batch_size>0)
...
def _handle_path(path, sess, low_profile=False, size=0):
    if path.endswith('.npz'):
        ...
        if low_profile:
            m, s = calculate_activation_statistics_from_files(files, sess, batch_size=256)
        else:
            x = np.array([process(fn, size) for fn in files])
            m, s = calculate_activation_statistics(x, sess, batch_size=256)
            # del x #clean up memory
    return m, s

def process(x, size=0):
    # read the file path x and resize
    # output the 0~255 numpy array
...
def calculate_fid_given_paths(paths, inception_path, low_profile=False, size=0):
    ...
    # with tf.Session() as sess: #TF 1.x
    with tf.compat.v1.Session() as sess: #TF 2.x
        # sess.run(tf.global_variables_initializer()) # TF 1.x
        sess.run(tf.compat.v1.global_variables_initializer()) #TF 2.x
        m1, s1 = _handle_path(paths[0], sess, low_profile=low_profile, size=size)
        m2, s2 = _handle_path(paths[1], sess, low_profile=low_profile, size=size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--size", default=0, type=int,
        help='To resize the images.')
    args = parser.parse_args()
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    fid_value = calculate_fid_given_paths(args.path, args.inception, low_profile=args.lowprofile, size=args.size)
    print("FID: ", fid_value)
    print("FID: ", fid_value)